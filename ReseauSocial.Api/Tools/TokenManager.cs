...
/*
        ...
        public string GenerateJWTClient(ClientBll client)
        {
        ...
        public string GenerateJWTDevelopper(DevelopperBll developper)
        {
            if (developper.Email is null)
                throw new ArgumentNullException();
            ...
            Claim[] myClaims = new[]
            {
                new Claim(ClaimTypes.Email, developper.Email),
                new Claim(ClaimTypes.Sid, developper.Id.ToString())
            };

            //Génération du token => Nuget : System.IdentityModel.Tokens.Jwt
            JwtSecurityToken tokenDevelopper = new JwtSecurityToken(
                claims: myClaims,
                expires: DateTime.Now.AddDays(1),
                signingCredentials: credentials,
                issuer: myIssuer,
            ...
            JwtSecurityTokenHandler tokenHandler = new JwtSecurityTokenHandler();
            return tokenHandler.WriteToken(tokenDevelopper);
        }*/
    }
}
